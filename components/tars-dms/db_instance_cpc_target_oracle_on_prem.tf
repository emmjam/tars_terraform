# Create the on prem CPC RDS DB for DMS
resource "aws_db_instance" "target_cpcdb_dms_onprem" {
  identifier                = "${local.csi}-cpcopdmstg"
  final_snapshot_identifier = "${local.csi}-cpcopdmstg-final"
