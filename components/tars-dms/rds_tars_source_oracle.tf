...
resource "aws_db_instance" "source_tarsdb_dms" {
  identifier                = "${local.csi}-tarsdmssc"
  final_snapshot_identifier = "${local.csi}-tarsdmssc-final"
  skip_final_snapshot       = "${var.source_dms_tars_rds_skip_final_snapshot}"
  allocated_storage         = "${var.source_dms_tars_rds_allocated_storage}"
  storage_type              = "${var.source_dms_tars_rds_storage_type}"
  engine                    = "${var.source_dms_tars_rds_engine}"
  engine_version            = "${var.source_dms_tars_rds_engine_version}"
  instance_class            = "${var.source_dms_tars_rds_instance_class}"
  username                  = "${var.source_dms_tars_rds_username}"
  password                  = "${var.source_dms_tars_rds_password}"
  port                      = "${var.source_dms_tars_rds_port}"
  publicly_accessible       = "${var.source_dms_tars_rds_public}"
  multi_az                  = "${var.source_dms_tars_rds_multi_az}"
  ...
  tags = "${merge(
    local.default_tags,
    map(
      "Name",     "${local.csi}-tarsdmssc"
      "AutoScale", var.source_dms_tars_rds_autoscale
    )
  )}"
}
