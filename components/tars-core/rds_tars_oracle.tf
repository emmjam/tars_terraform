...
resource "aws_db_instance" "tarsdb" {
  ...
  parameter_group_name    = "${var.project}-${var.component}-${var.environment}-${replace(var.tars_rds_instance_class, ".", "-")}"
  ...
  option_group_name       = "${aws_db_option_group.tars_core_oem.id}"
