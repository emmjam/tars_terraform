locals {
  ...
  driver_services_domain = "${ var.environment  == "prod" ? "dsa.dft.gov.uk" : (var.environment  != "prod" ? "${var.driver_service_cert}.${data.terraform_remote_state.acc.outputs.public_domain_name}" : "${var.driver_service_cert}.${data.terraform_remote_state.acc.outputs.public_domain_name}")}"
}