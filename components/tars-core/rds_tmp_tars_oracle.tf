...
resource "aws_db_instance" "tmp_tarsdb" {
  count  = "${var.uat_dbs_in_prod == "true" ? 1 : 0}"
  
  identifier                = "${local.csi}-tmp-tarsdb"
  final_snapshot_identifier = "${local.csi}-tmp-tarsdb-final"
  skip_final_snapshot       = "${var.tars_rds_skip_final_snapshot}"
  allocated_storage         = "${var.tmp_tars_rds_allocated_storage}"
  ...
  username                  = "${var.tmp_tars_rds_username}"
  password                  = "${var.tmp_tars_rds_password}"
  ...
  snapshot_identifier       = "${var.tmp_tars_rds_snapshot}"
