resource "aws_cloudwatch_metric_alarm" "rds_mis_freestoragespace_average" {
  count                =  var.rds_cw_metric_alarm_enabled ? 1 : 0
  alarm_name           = "${local.csi}-rds-mis-freestoragespace-average"
  ...
  dimensions  = {
    DBInstanceIdentifier    = aws_db_instance.misdb.id
  }
