resource "aws_route" "public_ctrl" {
  ...
  destination_cidr_block    = data.terraform_remote_state.ctrl.outputs.vpc_cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.ctrl.id
}

resource "aws_route" "ctrl_private_nat_base" {
  route_table_id            = data.terraform_remote_state.ctrl.outputs.rt_private_nat_id
  destination_cidr_block    = aws_vpc.vpc.cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.ctrl.id
}

resource "aws_route" "ctrl_private_base" {
  route_table_id            = data.terraform_remote_state.ctrl.outputs.rt_private_id
  destination_cidr_block    = aws_vpc.vpc.cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.ctrl.id
}
