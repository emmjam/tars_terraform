...
module "squidnat" {
  ...
  spot_price          = "${lookup(var.spot_pricing, var.squidnat_instance_type)}"

  # IAM for hiera
  hiera_kms_key_id        = "${data.terraform_remote_state.acc.hieradata_kms_key_id}"
  kms_hiera_user_policy_arn = "${data.terraform_remote_state.acc.hieradata_kms_key_user_policy_arn}"
