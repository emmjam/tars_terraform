module "tars_batch" {
  ...
  #TODO: smarts
  #  "tars-batch" breaks 64 characters when used with the
  ...
  lc_user_data     = "${data.template_cloudinit_config.wildfly-batch.rendered}"
  lc_spot_price    = "${lookup(var.rhel_spot_pricing, var.wildfly-batch_instance_type)}"
  ...
  asg_size_min               = "${var.wildfly-batch_asg_min_size}"
  asg_size_desired_on_create = "${var.wildfly-batch_asg_min_size}"
  asg_size_max               = "${var.wildfly-batch_asg_max_size}"
  asg_load_balancers         = []
