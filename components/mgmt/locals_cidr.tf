locals {
  sq_subnets_cidrs = [
    cidrsubnet(
      var.vpc_cidr,
      var.sonarqube["sq_subnets_newbits"],
      var.sonarqube["sq_subnets_netnum_root"],
    ),
  ]
  ...
  sqdb_subnets_cidrs = [
    cidrsubnet(
      var.vpc_cidr,
      var.sonarqube["rds_subnets_newbits"],
      var.sonarqube["rds_subnets_netnum_root"],
    ),
    cidrsubnet(
      var.vpc_cidr,
      var.sonarqube["rds_subnets_newbits"],
      var.sonarqube["rds_subnets_netnum_root"] + 1,
    ),
    cidrsubnet(
      var.vpc_cidr,
      var.sonarqube["rds_subnets_newbits"],
      var.sonarqube["rds_subnets_netnum_root"] + 2,
    ),
  ]
}
