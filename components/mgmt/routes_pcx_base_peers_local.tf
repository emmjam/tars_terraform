...
resource "aws_route" "private_pcx_base_peers_local" {
  ...
  vpc_peering_connection_id = element(
    data.aws_vpc_peering_connection.base_peers_local.*.id,
    count.index,
  )
  destination_cidr_block = element(
    data.aws_vpc_peering_connection.base_peers_local.*.cidr_block,
    count.index,
  )
}
...
resource "aws_route" "private_nat_pcx_base_peers_local" {
  ...
  count = length(var.base_peers_local) * length(aws_route_table.private_nat.*.id)
