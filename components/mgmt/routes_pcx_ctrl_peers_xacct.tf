...
resource "aws_route" "private_pcx_ctrl_peers_xacct" {
  ...
  destination_cidr_block = element(
    data.aws_vpc_peering_connection.ctrl_peers_xacct.*.cidr_block,
    count.index,
  )
}
...
resource "aws_route" "private_nat_pcx_ctrl_peers_xacct" {
  ...
  count = length(data.aws_vpc_peering_connection.ctrl_peers_xacct.*.id) * length(aws_route_table.private_nat.*.id)
