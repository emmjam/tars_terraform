...
resource "aws_route" "private_pcx_base_peers_xacct" {
...
resource "aws_route" "private_nat_pcx_base_peers_xacct" {
  ...
  count                     = "${
    length(data.aws_vpc_peering_connection.base_peers_xacct.*.id) * 
  ...
  vpc_peering_connection_id = "${element(data.aws_vpc_peering_connection.base_peers_xacct.*.id, ceil(count.index / length(aws_route_table.private_nat.*.id)))}"
  destination_cidr_block    = "${element(data.aws_vpc_peering_connection.base_peers_xacct.*.cidr_block, ceil(count.index / length(aws_route_table.private_nat.*.id)))}"
}

# For the public route table "public"
resource "aws_route" "public_pcx_base_peers_xacct" {
  count                     = "${length(data.aws_vpc_peering_connection.base_peers_xacct.*.id)}"
