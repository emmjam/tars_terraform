...
resource "aws_iam_role_policy_attachment" "cpc_batch_hieradata" {
  role       = "${module.cpc-batch.iam_role_name}"
  policy_arn = "${data.terraform_remote_state.acc.hieradata_kms_key_user_policy_arn}"
}

resource "aws_iam_role_policy_attachment" "cpc_batch_cloudwatch" {
    role       = "${module.cpc-batch.iam_role_name}"
